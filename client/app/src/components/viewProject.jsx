...
const ViewProject = ({ name}) => {
  ...
  const { getProjectDetails, updateProject,approvementor,pendingmentor,approverequest } = useStudentProject();
 ...
 useEffect(()=>{
  ...
  const handleEditButtonClick = async () => {
    try {
      if (isEditProject) {
        const data = await updateProject({ name, technology, userStory, updatedName });
    ...
    <div className="viewprojcontainer" id="viewprojcontainer">
      ...
      <div className="viewproj">
        <div className="projDetails">
          ...
          <button className="rowBtn" onClick={handleEditButtonClick}>
        ...
        <div className="projToggle">
          <div className="toggletable">
            ...
            <label htmlFor="toggle" className="toggleContainer" onClick={handleToggleClick}>
          ...
          {isApproved ? (
            <table className="approved-table">
              ...
              {/* Table body */}
              <tbody>
                {approvedData.slice((currentPage - 1) * itemsPerPage, currentPage * itemsPerPage).map((item, index) => (
                  <tr key={index}>
                    <td>{index + 1}</td>
                    <td>{item}</td>
                  </tr>
            ...
            <table className="pending-table">
              ...
              {/* Table body */}
              <tbody>
                {pendingData.slice((currentPage - 1) * itemsPerPage, currentPage * itemsPerPage).map((item, index) => (
                  <tr key={index}>
                    <td>{index + 1}</td>
                    <td>{item}</td>
                    <td>
                    <button className="rowBtn" onClick={() => handleApproveBtn(item)}>Approve</button>
          ...
          <div>
            {currentPage > 1 && (
              <button className="rowBtn" onClick={handlePrevPage}>Previous</button>
            )}
            {isApproved ? (
              totalPagesApproved > currentPage && (
                <button  className="rowBtn" onClick={handleNextPage}>Next</button>
              )
            ) : (
