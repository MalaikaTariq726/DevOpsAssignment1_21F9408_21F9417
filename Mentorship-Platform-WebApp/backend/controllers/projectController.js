...
const getMentorProjects = async (req, res) => {
  try {
    ...
    const projects = await mentordb.find({
      mentorId: mentorId,
      status: "approved",
    });
...
const getProjectDetailsByName = async (req, res) => {
  try {
    ...
    if (!student) {
      return res
        .status(404)
        .json({ error: "Student not found for the project" });
    }
...
const getProjectsByTechnology = async (req, res) => {
  try {
    ...
    if (projects.length === 0) {
      return res
        .status(404)
        .json({ error: "No projects found with the specified technology" });
    }
...
const approveproject = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Failed to approver the request of a project" });
  }
};
const getProjMentors = async (req, res) => {
  try {
    ...
    const mentorEmails = await userdb.find(
      { _id: { $in: mentorIds } },
      { email: 1 }
    );
...
const getPendingMentorsForStudent = async (req, res) => {
  try {
    ...
    for (const project of userProjects) {
      const mentors = await mentordb.find({
    ...
    if (pendingMentors.length === 0) {
      return res
        .status(404)
        .json({ error: "No pending mentors found for the user" });
    }
