...
const getStudentProjects = async (email) => {
  try {
    const response = await axios.get(`${BASE_URL}/project/studentprojects/${email}`);
...
const getProjectDetails = async (name) => {
  try {
    const response = await axios.get(`${BASE_URL}/project/getProjByName/${name}`);
    return response.data;
  } catch (error) {
...
const  updateProject=async(data)=>{
  try {
    const response = await axios.put(`${BASE_URL}/project/updateProject`,data);
    return response.data;
  } catch (error) {
...
const pendingmentor = async(name)=>{
  try {
    const response = await axios.get(`${BASE_URL}/project/pendingreq/${name}`);
    return response.data;
  } catch (error) {
    throw new Error(`Error fetching project details: ${error.message}`);
  }
...
const approverequest= async(data)=>{
  try {
    console.log(`${BASE_URL}/project/approve`);
    console.log(data);
...
const addProject =async(data,email)=>{
  try {
    console.log(email);
    console.log(data);
    const response = await axios.post(`${BASE_URL}/project/addProject/${email.email}`,data);
    return response;
  } catch (error) {
...
const contextValues = {
  getStudentProjects,getProjectDetails,updateProject,approvementor,pendingmentor,approverequest,getRequest,addProject,deleteProject,
 };
...
export { StudentProjectProvider as StudentProjectProvider,useStudentProject };
